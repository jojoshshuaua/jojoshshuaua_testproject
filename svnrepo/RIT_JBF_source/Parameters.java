...
public class Parameters extends Panel
{
    ...
    Parameters(Electrophoresis electrophoresis)
    {
        ...
        stdArray[std1Ref] = new Protein("Standard #1", "beta-Galactosidase", "b-gal", 0x1c58b, Color.blue);
        stdArray[std2Ref] = new Protein("Standard #2", "Ovalbumin", "oval", 42734, Color.yellow);
        stdArray[std3Ref] = new Protein("Standard #3", "Carbonic Anhydrase", "carb anh", 29011, Color.gray);
        stdArray[std4Ref] = new Protein("Standard #4", "Triose Phosphate Isomerase", "TIM", 26527, Color.green);
        stdArray[std5Ref] = new Protein("Standard #5", "Myoglobin", "Myo", 17183, Color.magenta);
        stdArray[std6Ref] = new Protein("Standard #6", "Lysozyme", "Lyso", 14296, Color.white);
        stdArray[std7Ref] = new Protein("Standard #7", "Trypsin Inhibitor", "BPTI", 6500, Color.red);
        ...
        headerSub1.add(new Label("ELECTROPHORESIS PARAMETERS"));
        headerSub2.add(new Label("Animation Speed "));
        ...
        labelPanel1.add(new Label("Unknown Proteins"));
        labelPanel1.add(new Label("% Acrylamide"));
        labelPanel2.add(new Label("Standards"));
        ...
        controlPanel.add(new Button("Add Standard"));
        controlPanel.add(new Button("Add Sample"));
        controlPanel.add(new Button("Start Run"));
        controlPanel.add(new Button("Stop Run"));
        add(headerPanel);
        add(dropPanel);
        add(selectionPanel2);
        add(voltagePanel);
    ...
    {
        if("Add Standard".equals(obj))
        {
            parent.addStandard();
            return true;
        }
        if("Add Sample".equals(obj))
        {
            parent.addSample();
            return true;
        }
        if("Start Run".equals(obj))
        {
            parent.startRun(stdArray, selectedSample, dye1, dye2);
            return true;
        }
        if("Stop Run".equals(obj))
        {
            parent.stopRun();
